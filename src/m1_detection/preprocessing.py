...
#      the full verbatim run in esa-adb/data/preprocessed).
...
# CHANGE 2: separate output dir (was "../../data/preprocessed") to avoid colliding
# with the full ESA run. current_dir = repo/src/m1_detection -> repo/data/preprocessed_subset.
...
def process_dataset(dm: DatasetManager, dataset_name: str, split_at: str, resampling_rule=pd.Timedelta(seconds=18)):
    ...
    extension = ".zip"
    # CHANGE 1: restrict to Mission2 lightweight subset (channels 18-28) instead of
    # globbing all 100 channels. Each channel is processed independently, so the
    # 18-28 outputs are identical to the full run; only the column set shrinks.
    all_parameter_names = sorted([f"channel_{i}" for i in range(18, 29)])
